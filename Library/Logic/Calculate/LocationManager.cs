...
namespace NakshatraaDiaries.Library
{
    ...
    public class LocationManager
    {
        ...
        {
            NakshatraaDiaries, Azure, Google, IpData, CPU,
            LocalFile
        }
        ...
        public async Task<GeoLocation> AddressToGeoLocation(string userInputAddressRaw)
        {
            ...
            {
                {APIProvider.NakshatraaDiaries, AddressToGeoLocation_NakshatraaDiaries},
            ...
            foreach (var row in geoLocationProviders)
            {
                ...
                // only add to cache if not empty and not NakshatraaDiaries
                var isNotEmpty = parsedGeoLocation.Name() != GeoLocation.Empty.Name();
                var apiProvider = row.Key;
                var isNotNakshatraaDiaries = apiProvider != APIProvider.NakshatraaDiaries;
                if (isNotEmpty && isNotNakshatraaDiaries)
                {
        ...
        public async Task<List<GeoLocation>> SearchAddressToGeoLocation(string userInputAddressRaw)
        {
            ...
            var geoLocationProviders = new Dictionary<APIProvider, Func<string, Task<GeoLocationRawAPI>>>
            {
                {APIProvider.NakshatraaDiaries, SearchAddressToGeoLocation_NakshatraaDiaries},
        ...
        public async Task<GeoLocation> IpAddressToGeoLocation(string ipAddress)
        {
            ...
            var geoLocationProviders = new Dictionary<APIProvider, Func<string, Task<GeoLocationRawAPI>>>
            {
                {APIProvider.NakshatraaDiaries, IpAddressToGeoLocation_NakshatraaDiaries},
        ...
        public async Task<GeoLocation> CoordinatesToGeoLocation(string latitudeStr, string longitudeStr)
        {
            ...
            {
                {APIProvider.NakshatraaDiaries, CoordinatesToGeoLocation_Vedastro},
        ...
        public async Task<string> GeoLocationToTimezone(GeoLocation geoLocation, DateTimeOffset stdTimeAtLocation)
        {
            ...
            {
                {APIProvider.NakshatraaDiaries, GeoLocationToTimezone_Vedastro},
            ...
            foreach (var row in geoLocationProviders)
            {
                ...
                if (isNotEmpty && isNotNakshatraaDiaries && isNotCpu)
                {
                    //NOTE: to support local development, since saving to azure db will be unavailable
                    try
                    {
        ...
        private async Task<GeoLocationRawAPI> IpAddressToGeoLocation_NakshatraaDiaries(string ipAddress)
        {
        ...
        /// </summary>
        private async Task<GeoLocationRawAPI> AddressToGeoLocation_NakshatraaDiaries(string userInputAddress)
        {
        ...
        private async Task<GeoLocationRawAPI> SearchAddressToGeoLocation_NakshatraaDiaries(string userInputAddress)
        {
            //do direct search for address in name field
        ...
        /// NOTE: The only place so far Google API outside NakshatraaDiaries API
        /// </summary>
        ...
        private static async Task<GeoLocationRawAPI> IpAddressToGeoLocation_Azure(string ipAddress)
        {
            ...
            //            var webResult = await NakshatraaDiaries.Library.Tools.ReadFromServerJsonReply(url);
        ...
        private static dynamic TryParseGoogleTimeZoneResponse(DateTimeOffset timeAtLocation, GeoLocation geoLocation, XElement apiResultPayload)
        {
            ...
            try
            {
                ...
                var timezoneText = NakshatraaDiaries.Library.Tools.TimeSpanToUTCTimezoneString(parsedData.StandardOffset);
        ...
        private static dynamic TryParseAzureSearchAddressResponse(JToken geocodeResponseJson, string userInputAddress)
        {
            try
            {
                ...
                //package such that it look like it came from NakshatraaDiaries db for easy interop
